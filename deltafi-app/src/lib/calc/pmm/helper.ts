...
export class PMMHelper {
  ...
  public querySellBase(amount: BigNumber, state: PMMState): BigNumber {
    try {
      ...
      // const lpFee = mtFee.multipliedBy(state.lpFeeRate)

      const quote = result.minus(mtFee);
  ...
  private ROneSellBase(amount: BigNumber, state: PMMState): BigNumber {
    return solveQuadraticFunctionForTrade(state.Q0, state.Q0, amount, state.OraclePrice, state.k);
  }
  ...
  private RBelowSellBase(amount: BigNumber, state: PMMState): BigNumber {
    return solveQuadraticFunctionForTrade(state.Q0, state.Q, amount, state.OraclePrice, state.k);
  }
}
...
export const integrate = (
  ...
  let penalty = V0.multipliedBy(V0).div(V1).div(V2).multipliedBy(k);
